...
fun Service.createNotification(notificationMsg: String) {
    ...
    val builder = NotificationCompat.Builder(this, NOTIFICATION_CHANNEL)
        ...
        .setOngoing(true)
        .addAction(NotificationCompat.Action(android.R.drawable.alert_light_frame, "Disconnect", disconnectPendingIntent))
 ...
 fun Service.createMessageNotification(message:String){
    ...
    val builder = NotificationCompat.Builder(this, NOTIFICATION_CHANNEL)
        ...
        .setContentIntent(PendingIntent.getActivity(this,2,notificationIntent,PendingIntent.FLAG_NO_CREATE))
    // @drawable/ic_notification created with Android Studio -> New -> Image Asset using @color/colorPrimaryDark as background color
    // Android < API 21 does not support vectorDrawables in notifications, so both drawables used here, are created as .png instead of .xml
    val notification = builder.build()
    startForeground(NOTIFY_MANAGER_START_FOREGROUND_SERVICE, notification)
